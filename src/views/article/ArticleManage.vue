...
<template>
  <page-container title="文章管理">
    ...
    <el-form inline display="flex" justify-content="space-between">
      <el-form-item label="文章分类:">
        <!-- 在Vue2中，v-model是 :value 和 @input 的简写 -->
        <!-- 而在Vue3中，v-model是 :modelValue 和 @update:modelValue 的简写，
             所以当父组件接收到子组件传递过来的修改信号，@update:modelValue就会触发，然后根据子组件传递的值对:modelValue进行修改完成双向绑定 -->
    ...
    <el-table :data="articleList" v-loading="loading">
      <el-table-column label="文章标题" prop="title">
        <!-- 1.利用作用域插槽自定义列表内容 row 可以获取当前行的数据 => v-for 遍历 item -->
