 ...
 * @description 基于 AbortController 管理未完成的 GET 请求，支持路由切换时统一取消
 * @version 1.0.0
 *
 * @remarks
 * **设计目标：**
 * - 用户切换路由时，取消上一个页面未完成的 GET 请求，避免无意义的网络消耗与状态污染
...
/**
 * 注册请求到 pending 池
 * @description 仅管理 GET 请求；为请求注入 AbortController signal
...
/**
 * 取消所有未完成的 GET 请求
 * @description 路由切换时调用，abort 后会触发 axios ERR_CANCELED 错误
 */
